#!/usr/bin/env node
/*
  Universal Scraper v2 with Puppeteer support
  Extended version that can use Puppeteer for selected portals while keeping others as fetch/CSS.
  Puppeteer portals: gewobag, inberlinwohnen (marked with usePuppeteer: true in sites-config-puppeteer.json)
...
try {
  ...
  console.error('❌ Puppeteer dependencies missing. Run: npm install puppeteer-extra puppeteer-extra-plugin-stealth');
  process.exit(1);
}

const RESULTS_FILE = '/home/node/.openclaw/wohnungen-results-v2.json';
const CONFIG_FILE = path.join(__dirname, 'sites-config-puppeteer.json');
...
async function scrapeWithPuppeteer(siteId, cfg) {
  // Delegation based on site type
  if (cfg.type === 'inberlin') {
    return await scrapeInberlinWithPuppeteer(cfg);
  }
  // Default CSS-based scraping with Puppeteer
  const apartments = [];
  const seen = new Set();
  ...
  try {
    ...
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      if (pageNum > 1) {
        if (cfg.pagination) {
          url = cfg.baseUrl + cfg.pagination.replace('{page}', String(pageNum)).replace('{pageIndex}', String((cfg.pageIndexStartsAt || 0) + (pageNum - 1)));
        } else {
          url = cfg.baseUrl;
        }
      }

      process.stdout.write(`📄 ${cfg.name} - Seite ${pageNum} (Puppeteer): ${url}\n`);

      await page.goto(url, { waitUntil: 'networkidle2', timeout: 30000 });

      // Wait for item selector or timeout
      try {
        await page.waitForSelector(cfg.itemSelector || 'article', { timeout: 10000 });
      } catch (e) {
        process.stdout.write(`   ⚠️  Item selector not found, maybe no items?\n`);
      }
      ...
      // Stop when the last page is not full (if pageSize is defined)
      if (cfg.pageSize && newCount < cfg.pageSize) {
        process.stdout.write(`   ℹ️  Page less than pageSize, assuming end of list.\n`);
        break;
      }
...
async function scrapeInberlinWithPuppeteer(cfg) {
  const out = [];
  const seen = new Set();
  const browser = await puppeteer.launch({
    headless: 'new',
  ...
  try {
    ...
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      // Wait for aria-label elements
      await page.waitForSelector('[aria-label*="Wohnungsangebot"]', { timeout: 10000 }).catch(() => {
        process.stdout.write(`   ⚠️  Keine Wohnungsangebote gefunden.\n`);
      });
      ...
      $('[aria-label*="Wohnungsangebot"]').each((i, el) => {
        ...
        const key = link || JSON.stringify([parsed.rooms, parsed.size, parsed.price, parsed.address, parsed.district]);
        if (seen.has(key)) return;
        seen.add(key);
        out.push({
          source: cfg.name,
          title: `Wohnung in ${parsed.district || ''}`.trim(),
          rooms: parsed.rooms,
          size: parsed.size,
          price: parsed.price,
      ...
      if (cfg.pageSize && added < cfg.pageSize) break;
...
async function scrapeHowoge(cfg){
  ...
  // HOWOGE endpoint currently does NOT paginate by page/limit reliably.
  // Best effort strategy:
  // 1) Fetch the "all" list (immoobjects + projectteaser)
  // 2) Fetch each kiez filter from the public form options and union unique results
  // This typically recovers a few offers that don't show up in the unfiltered response.

  const fetchHowogeJson = async (params) => {
  ...
  const pushTeasersAndProjects = async (teasers) => {
    let projectApartmentsCount = 0;

    for (const t of teasers || []) {
      await sleep(cfg.delayMs || 300);
  ...
  const dedupOut = (arr) => {
    const unique=[];
    for (const a of arr) {
      const key = a.link || JSON.stringify([a.title,a.price,a.size,a.rooms,a.address,a.district]);
  ...
  process.stdout.write(`📄 ${cfg.name} - API (unfiltered): ${cfg.baseUrl}\n`);
  const baseParams = {
    'tx_howrealestate_json_list[page]': 1,
    'tx_howrealestate_json_list[limit]': limit,
    'tx_howrealestate_json_list[lang]': 'de',
    'tx_howrealestate_json_list[rooms]': '',
    'tx_howrealestate_json_list[wbs]': '',
  };
  const base = await fetchHowogeJson(baseParams);
  if (base) {
    pushImmoObjects(base.immoobjects);
    await pushTeasersAndProjects(base.projectteaser);
  }
  ...
  for (const k of kiezes) {
    await sleep(cfg.delayMs || 300);
    process.stdout.write(`   🧩 kiez=${k}\n`);
    const j = await fetchHowogeJson({
      ...baseParams,
      'tx_howrealestate_json_list[kiez][]': k,
    });
    if (!j) continue;
    pushImmoObjects(j.immoobjects);
  }

  // final dedup
  const final = dedupOut(out);
  return final;
}
...
async function scrapeApi(cfg){
  ...
  if ((cfg.id === 'stadtundland' || /stadt\s*&\s*land/i.test(cfg.name))) {
    ...
    const pushWohnungen = (arr) => {
      for (const it of arr) {
        const immoType = String(it?.details?.immoType || '').toLowerCase();
        const immoSubType = String(it?.details?.immoSubType || '').toLowerCase();

        // Keep ALL object types from Stadt & Land (Wohnungen + Stellplätze/Garagen).
        // This avoids silently missing inventory when their API "count" includes non-apartment objects.
        // You can filter later by `immoType === 'wohnung'`.
        all.push({
          source: cfg.name,
          immoType,
          immoSubType,
          ...
          immoNumber: it.details?.immoNumber || null,
          // Stadt&Land detail URLs changed; the old /immobilien/expose/<immoNumber>/ route 404s.
          // Until we derive the new per-object route, link to the search entry point instead of a broken URL.
          link: it.details?.immoNumber ? `https://stadtundland.de/wohnungssuche#${encodeURIComponent(it.details.immoNumber)}` : null,
          scraped: new Date().toISOString(),
        });
      }
    };
    ...
    for (const it of list) {
      ...
      const link = (() => {
        ...
        if (isDeuwoSchema && it.slug) {
          // Both Deutsche Wohnen + Vonovia exposes are served under Vonovia's search routes.
          // Old patterns like /immobilien/<slug> or deutsche-wohnen.com/expose/<slug> often 404.
          const base = 'https://www.vonovia.de/zuhause-finden/immobilien';
...
async function scrapeInberlin(cfg){
  ...
  for (let page=1; page<=maxPages; page++){
    ...
    // Stop when the last page is not full (prevents truncation when maxPages is too low).
    if (cfg.pageSize && added < cfg.pageSize) break;
...
async function main(){
  ...
  console.log('🚀 Universal Scraper v2 with Puppeteer gestartet');
  ...
  for (const s of active) {
    ...
    try {
      ...
      if (s.usePuppeteer === true) {
        apts = await scrapeWithPuppeteer(s.id, s);
      } else if (s.type === 'css') apts = await scrapeCssSite(s.id, s);
